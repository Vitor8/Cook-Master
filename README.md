...
Após clonar o repositório, para ver a aplicação em execução, digite o comando `npm start` em seu terminal.

---

## Desenvolvimento

Para desenvolver o projeto utilizei as camadas da aplicação (Models, Service e Controllers).
...
Também é possível adicionar uma imagem à uma receita, utilizando o upload de arquivos fornecido pelo `multer`.
...
O banco possui duas coleções: usuários e receitas.
...
O nome da coleção de é: `recipes`.
...
Meu código segue o padrão definido pelo Lint. Para comprovar rode o comando `npm run lint` 
